...
##################################
#### Test with codes from CDC #### 
##################################

ICD10_CDC21 <- fread("Analysis and QA/Supplementary table/ICD-10 codes/icd10cm_codes_2021.csv",
                     header=TRUE, sep=",", check.names=T)

#ICD10_CDC21_sample <- ICD10_CDC21[sample(1:nrow(ICD10_CDC21),100,replace=FALSE),]
ICD10_CDC21_sample <- ICD10_CDC21[,]

output_cdc <- ICD10_CDC21_sample$ICD.10.codes %>%
  pblapply(., icd10_to_chapter) %>%
  flatten_chr(.)

output_cdc_counts <- plyr::count(as.data.frame(output_cdc), 'output_cdc')
filter(output_cdc_counts,freq<100)

# output_cdc <- ICD10_CDC21_sample$ICD.10.codes %>%
#   map(icd10_to_chapter) %>%
#   flatten_chr(.)

no_match_output_cdc <- ICD10_CDC21_sample[which(output_cdc=="No match with ICD-10 chapters"),]

#2576/72616*100 (3.5%)

unmatched_ICD10 <- no_match_output_cdc$ICD.10.codes %>%
  substr(., 1, 3) %>%
  unique(.)
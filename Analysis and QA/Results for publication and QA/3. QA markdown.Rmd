...
#Order of months
month.lookup <- data.frame(month.number=c("01","02","03","04","05","06","07","08","09","10","11","12"),month.abbr=month.abb)
...
#Color palettes for age statistics
...
pooled.drop.april.outae <- table1.all.partners.pcts %>% 
  filter(.,strata=="overall",year_month=="2020-04",partner!="Wales",
         breakdown.level %in% c("outpatient_attendance","AE_attendance")) %>%
...
drop.april.outae <- table1.all.partners.pcts %>%
  filter(.,strata=="overall",year_month=="2020-04",partner!="Wales",
         breakdown.level %in% c("outpatient_attendance","AE_attendance")) %>%
  select(.,breakdown.level,partner,year_month,
...
```{r elective.apr.pct.change.pooled, echo=TRUE, message=FALSE, warning=FALSE}

pooled.drop.april.ael <- table1.all.partners.pcts %>% filter(.,strata=="overall",
...
drop.april.ael <- table1.all.partners.pcts %>% filter(.,strata=="overall",
                            year_month=="2020-04",partner!="Leeds",
                            breakdown.level %in% c("admissions_electives")) %>%
  select(.,breakdown.level,partner,year_month,
         number.events.sdc_l1,number.events.sdc,abs_change_12m) %>%
...
```{r emergency.pooled.apr.pct.change, echo=TRUE, message=FALSE, warning=FALSE}

drop.april.pooled.ae <- table1.all.partners.pcts %>% filter(.,strata=="overall",
...
```{r emergency.apr.pct.change, echo=TRUE, message=FALSE, warning=FALSE}

drop.april.ae <- table1.all.partners.pcts %>%
  filter(.,strata=="overall",year_month=="2020-04",partner!="Leeds",
  breakdown.level %in% c("admissions_emergency")) %>%
  select(.,breakdown.level,partner,year_month,
         number.events.sdc_l1,number.events.sdc,
         abs_change_12m) %>%
...
pooled.drop.april.by.age <- table1.all.partners.pcts %>% filter(.,breakdown=="age",
                            year_month=="2020-04",partner!="Wales",
...
```{r apr.drop.by.age.pooled.ng, echo=TRUE, message=FALSE, warning=FALSE}
pooled.nl.drop.april.by.age <- table1.all.partners.pcts %>%
  filter(.,breakdown=="age",year_month=="2020-04",partner!="Leeds",
...
```{r apr.drop.by.age.pooled.ngnl, echo=TRUE, message=FALSE, warning=FALSE}
pooled.ngnl.drop.april.by.age <- table1.all.partners.pcts %>%
  filter(.,breakdown=="age",year_month=="2020-04",partner!="Grampian",partner!="Leeds",
  breakdown.level %in% c("admissions_emergency"), strata %in% c("0-29","70+")) %>%
  select(.,partner,strata,breakdown.level,year_month,
         number.events.sdc_l1,number.events.sdc,
         abs_change_12m) %>%
  group_by(breakdown.level,strata) %>%
...
drop.april.by.age <- table1.all.partners.pcts %>%
  ...
  !(partner=="Leeds"&breakdown.level=="admissions_electives"),
  !(partner=="Leeds"&breakdown.level=="admissions_emergency"),
  !(partner=="Wales"&breakdown.level=="outpatient_attendance"),
  !(partner=="Wales"&breakdown.level=="AE_attendance"),
...
covid.adm.data <- table1.all.partners %>%
  filter(.,strata=="overall",breakdown.level=="admissions_all",
    partner!="Leeds",year_month>="2020-03") %>%
  select(.,breakdown.level,partner,year_month,number.patients.sdc,
         number.patients.covid,total.patients)
...
total.deaths.cum.pooled <- table3.all.partners %>% filter(.,
                                  location=="anywhere",
